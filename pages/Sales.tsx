...
interface CartContentsProps {
  ...
  updatePriceUSD: (id: string, newPrice: number) => void;
  exchangeRate: number;
  subtotalUSD: number;
  subtotalARS: number;
  channel: SaleChannel;
  setChannel: (c: SaleChannel) => void;
  customer: string;
...
export const Sales: React.FC = () => {
  ...
  const addToCart = (product: Product) => {
    ...
    if (existing) {
      ...
      const suggestedPriceUSD =
        product.avgCostUSD * (1 + product.targetMargin / 100);
      setCart([
        ...cart,
        { product, quantity: 1, priceUSD: suggestedPriceUSD },
      ]);
    }
  };
  ...
  const handleCheckout = () => {
    ...
    recordSale(saleItems, channel, customer || undefined);
    setCart([]);
    setCustomer('');
    setSuccessMsg(
      `Venta registrada: $${totals.subtotalARS.toLocaleString('es-AR')}`
    );
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="bg-slate-100 text-slate-600 text-[10px] font-medium px-2 py-1 rounded-full uppercase tracking-wide">
                      Stock: {product.currentStock}
                    </span>
                  </div>
                  <h3 className="font-bold text-slate-900 text-lg mb-4 leading-tight group-hover:text-amber-600 transition-colors">
                    {product.name}
                  </h3>
                  <div className="pt-3 border-t border-slate-50 flex justify-between items-end">
                    ...
                    <div className="text-right">
                      ...
                      <div className="font-mono font-medium text-slate-600 text-lg">
                        ${suggestedUSD.toFixed(0)}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <div className="font-bold text-lg">
                  ${subtotalARS.toLocaleString('es-AR')}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-t-3xl shadow-2xl w-full h-[85vh] relative flex flex-col animate-fade-in">
              <div className="p-4 border-b border-slate-100 flex items-center justify-between">
                <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                  <ShoppingCart size={20} /> Resumen de la venta
                </h2>
                <button
                  onClick={() => setIsMobileCartOpen(false)}
...
/** Contenido reutilizable del carrito (desktop y móvil). */
      ...
      <div className="flex-1 space-y-4 overflow-y-auto min-h-0">
        {cart.length === 0 ? (
          ...
          cart.map((item) => (
            <div
              key={item.product.id}
              className="flex flex-col gap-3 p-4 border border-slate-100 rounded-xl bg-slate-50/50"
            >
              ...
              <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-slate-100 text-slate-600 font-medium transition-colors"
                  >
                    -
                  </button>
