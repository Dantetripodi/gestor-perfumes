...
export const Dashboard: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    const totalRevenueARS = sales.reduce((acc, sale) => acc + sale.totalTotalARS, 0);
    
    const grossProfitARS = sales.reduce((acc, sale) => {
        const saleCostUSD = sale.items.reduce((itemAcc, item) => itemAcc + (item.unitCostAtSaleUSD * item.quantity), 0);
        const saleCostARS = saleCostUSD * sale.exchangeRateUsed; 
        return acc + (sale.totalTotalARS - saleCostARS);
    }, 0);

    const totalStockValueUSD = products.reduce((acc, p) => acc + (p.currentStock * p.avgCostUSD), 0);
    ...
    return {
      ...
      totalStockValueUSD,
      totalStockValueARS,
      margin: totalRevenueARS > 0 ? (grossProfitARS / totalRevenueARS) * 100 : 0
    };
  ...
  const bestSellers = useMemo(() => {
      ...
      sales.forEach(sale => {
          sale.items.forEach(item => {
              if(!counts[item.productId]) counts[item.productId] = { name: item.productName, qty: 0 };
  ...
  const handleGetInsight = async () => {
    ...
    const result = await generateBusinessInsight(sales, products, exchangeRate);
    setInsight(result);
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        >
          {loadingInsight ? <Sparkles size={18} className="animate-spin" /> : <Sparkles size={18} className="text-amber-400" />}
          <span className="font-medium">{loadingInsight ? 'Analyzing...' : 'AI Advisor'}</span>
        </button>
      </div>
      ...
      {insight && (
        <div className="bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 p-6 rounded-2xl shadow-sm animate-fade-in relative overflow-hidden">
          ...
          <h3 className="text-indigo-900 font-bold mb-3 flex items-center gap-2 relative z-10">
            <Sparkles size={18} className="text-indigo-600"/> Insight Report
          </h3>
          <div className="prose prose-sm prose-indigo text-slate-700 markdown-body relative z-10">
            <pre className="whitespace-pre-wrap font-sans bg-transparent p-0 text-slate-700">{insight}</pre>
          </div>
          <button onClick={() => setInsight(null)} className="text-xs font-semibold text-indigo-600 mt-4 hover:text-indigo-800 z-10 relative">Close Report</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-2xl font-bold text-slate-900 tracking-tight">
                ${(metrics.totalRevenueARS / 1000).toFixed(1)}k <span className="text-sm font-normal text-slate-400">ARS</span>
            </h3>
          </div>
        </div>
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center gap-1 text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg">
                    {metrics.margin.toFixed(0)}% Margin
                </div>
            </div>
            <p className="text-sm font-medium text-slate-500 mb-1">Gross Profit</p>
            <h3 className="text-2xl font-bold text-slate-900 tracking-tight">
                ${(metrics.grossProfitARS / 1000).toFixed(1)}k <span className="text-sm font-normal text-slate-400">ARS</span>
            </h3>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group relative overflow-hidden">
           ...
           <div className="relative z-10">
            ...
            <h3 className="text-2xl font-bold text-slate-900 tracking-tight">
                ${metrics.totalStockValueUSD.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} <span className="text-sm font-normal text-slate-400">USD</span>
            </h3>
          </div>
        </div>
        ...
        <div className="bg-slate-900 p-6 rounded-2xl shadow-lg shadow-slate-900/10 text-white relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm font-medium text-slate-400 mb-1">Top Product</p>
            <h3 className="text-xl font-bold text-white tracking-tight truncate">
                {bestSellers[0]?.name || 'No Sales Yet'}
            </h3>
            <p className="text-xs text-slate-400 mt-1">
                {bestSellers[0]?.qty || 0} units sold
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
            Revenue Trend <span className="text-xs font-normal text-slate-400 bg-slate-100 px-2 py-1 rounded-full">Last 7 Days</span>
          </h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={last7DaysSales} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="amount" stroke="#f59e0b" strokeWidth={3} fillOpacity={1} fill="url(#colorRevenue)" activeDot={{r: 6, strokeWidth: 0, fill: '#b45309'}} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h3 className="text-lg font-bold text-slate-800 mb-6">Top Sellers</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={bestSellers} layout="vertical" margin={{ top: 0, right: 30, left: 0, bottom: 0 }}>
                ...
                />
                <Bar dataKey="qty" fill="#1e293b" radius={[0, 6, 6, 0]} barSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
